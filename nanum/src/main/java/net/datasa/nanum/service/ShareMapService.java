...
public class ShareMapService {
    ...
    /**
     * service.html의 지도에 표시하기 위해 거래가 완료된 게시글 가져오기
     * 
     * @param swLat
     * @param swLng
     * @param neLat
     ...
     * @return
     */
    public List<ShareBoardDTO> serviceList(double swLat, double swLng, double neLat, double neLng) {
        // entityList 에 게시글들을 받는다.
        List<ShareBoardEntity> entityList = shareBoardRepository.findServiceList(swLat, swLng, neLat, neLng);
        // DTO로 변환할 리스트 생성
        List<ShareBoardDTO> dtoList = new ArrayList<>();
        // entityList를 DTO로 변환해서 dtoList에 저장
        for (ShareBoardEntity entity : entityList) {
